...
class PaymentPendingEmail
    ...
    public sub Class_Initialize()
        ...
        s_emailTemplate = settings(SETTING_EMAILTEMPLATE_PAYMENTPENDING)
    end sub
    ...
    private function GetBody()
        ...
        set oStream = oFSO.OpenTextFile(Server.MapPath(PathCombine(Settings(SETTING_EMAILTEMPLATE_PATH), Settings(SETTING_EMAILTEMPLATE_FILE_PAYMENTPENDING))))
    ...
    private function ReplaceBodyConstants(BodyContent)
        ...
        BodyContent = NullableReplace(BodyContent, "{{Subject}}", Settings(SETTING_EMAILSUBJECT_PAYMENTPENDING))
    ...
    public sub SendById(nEventId)
        ...
        if isNull(mo_Event.ContactEmailAddress) or len(mo_Event.ContactEmailAddress) = 0 then
            ...
            myPrimaryEmail.Send mo_Event.ContactName, mo_Event.ContactEmailAddress, Settings(SETTING_EMAILSUBJECT_PAYMENTPENDING), body
            mySecondaryEmail.SendToReservation mo_Event.ContactName, mo_Event.ContactEmailAddress, Settings(SETTING_EMAILSUBJECT_PAYMENTPENDING), body
        end if        
    end sub
    ...
    public sub SendByObject(oEvent)
        if isNull(oEvent.ContactEmailAddress) or len(oEvent.ContactEmailAddress) = 0 then
            ...
            myPrimaryEmail.Send oEvent.ContactName, oEvent.ContactEmailAddress, Settings(SETTING_EMAILSUBJECT_PAYMENTPENDING), body
            mySecondaryEmail.SendToReservation oEvent.ContactName, oEvent.ContactEmailAddress, Settings(SETTING_EMAILSUBJECT_PAYMENTPENDING), body
        end if
    end sub
end class
%>